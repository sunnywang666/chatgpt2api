...
直接兼容 `/v1/chat/completions`、`/v1/responses`、`/v1/messages`、`/v1/search` 和未走公开持久图片适配的兼容生图入口，目前只保存响应或流前缀；没有把协议的原上游会话/消息游标完整写入这些收据，不能从上游恢复其 UNKNOWN。调度及手动读取都跳过这类收据的通用文字恢复，避免误用不完整游标。相同ID重连只重读已保存字节，不补全未知尾部、不伪造成功标记、不再发生成；这项限制持续开放，不能称作本批所有兼容路径的原结果恢复已经完成。带公开图片标记的同步图片接口沿 `/api/image-tasks` 原收据，仍享有该持久接口的原结果查询/下载恢复。
